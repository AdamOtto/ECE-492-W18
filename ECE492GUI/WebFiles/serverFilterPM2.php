<?php
	...
	$sql ="SELECT `StationName`,`Latitude`,`Longitude`,`Dust 2.5`, Date
	...
	echo "StationName,Latitude,Longitude,Dust2.5,Date\n";
	if ($resultCheck > 0) {
	    while($row = mysqli_fetch_assoc($result)) {
	        echo $row['StationName'] ."," . $row["Latitude"] . "," . $row["Longitude"] ."," . $row["Dust 2.5"] . "," . $row["Date"] . "\n";
	   }
	}
		else{
		echo "error";
