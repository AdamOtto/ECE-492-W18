...
void SMPWM01A::begin() {
  ...
  OCR1A  = 62500*7.5;       						// compare A register value
  ...
  PCMSK0 |= (1<<PCINT1);  // want pin  9
  PCMSK0 |= (1<<PCINT2);  // want pin  10
  PCIFR  |= (1<<PCIF0);   	// clear any outstanding interrupts
  PCICR  |= (1<<PCIE0);   	// enable pin change interrupts for 9&10
